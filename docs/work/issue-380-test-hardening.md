...
- **2026-07-26 (correction to the 2026-06-01 CRLF entry)** — The entry above says "The committed blobs are LF — exactly what Linux CI lints — so CI lint passes (Linux is authoritative)". That was accurate when written; [#1145](https://github.com/DysTelefonica/dysflow/pull/1145) made it stale. CI has no Linux runner any more: `.github/workflows/ci.yml` runs the `Quality gates` job — Biome (`pnpm lint`) included — on `windows-latest`, so Linux is not authoritative for lint or for line endings. The same PR added `.gitattributes` with `* text=auto eol=lf`, which gives every checkout an LF working tree on Windows too, so the CRLF-versus-LF split that entry describes no longer exists. The conclusion still stands for a different reason: do not run `pnpm lint:fix` to silence line-ending noise — fix the checkout instead (`git add --renormalize .` on a stale clone).
