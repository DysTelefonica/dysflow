...
*   **How to read results**: use compact `summaryStructured`, `summaryByCategory`, `actionableOk`, and `recommendedAction` for the decision. Use `bulkImportable` directly as `import_modules.moduleNames` and `bulkExportable` directly as `export_modules.moduleNames`. The category maps are aggregate counts; they do not identify module membership. Never align those totals with names from another array or report. Pass `diagnostic:true` once and read `actionableDifferent[]` / `nonActionableDifferent[]` when membership matters. Raw `ok` can remain false for non-actionable noise; read `nonActionableByCategory` to see which noise it was — `caseOnly` and `whitespaceOnly` are non-actionable. `bothChanged` / `manual_merge` still requires human resolution.
...
*   **Step 1 — `verify_code` (semantic mode, no `strict:true`)**:
    ...
    ```
    Compact response shape (excerpt):
    ```json
    {
      "ok": false,
      "summaryStructured": {
    ...
    For the full diagnostic payload, including classified arrays and snippets:
    ```json
    {
      "projectId": "my-project",
      "diagnostic": true
...
*   **Notes**:
    ...
    *   Strict mode keeps byte/text-exact classifier semantics. Pair it with `diagnostic:true` when you need the raw strict evidence.
