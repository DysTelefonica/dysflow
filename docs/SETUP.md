...
Install the CLI from the signed asset on the
[current GitHub release](https://github.com/DysTelefonica/dysflow/releases/latest).
The root package is private and is not a public npm installation channel. The
separate public `@aroman22/dysflow-pi` package is installed only through the
Dysflow command below so ownership remains explicit.
...
configuration. Pi users should follow the
[Pi-native integration guide](./pi-native-integration.md); that page is the
single source for Pi reconciliation, rendering, update, and uninstall behavior.
