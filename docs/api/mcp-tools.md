...
| Path normalization | Forward slashes, backslashes, and mixed separators are accepted on Windows; relative paths resolve against the worktree root; case-insensitive comparison. A relative override is absolutized by the resolver itself, so passing `destinationRoot: "src"` when `src` is also the configured root writes to `<root>/src/`, never `<root>/src/src/` (#1478). Absolute overrides are kept byte-identical so the containment check still sees the caller's path. | `src/core/config/execution-target.ts:36-60`, `test/core/config/execution-target-relative-destination-root.test.ts`, `src/adapters/config/project-config-diagnostic.ts:460-470` |
...
* **Progressive parameters**: `view:"compact"` returns the bounded projection; `view:"full"` opts into every block. Compact always includes base identity/gates and, by default, compact `tools`, `sharedBlockSupport`, `effectiveDryRunDefault`, `migrationNotes`, and the resolver-provided three-key `documentationBundle`. `include` selects blocks and `toolNames` filters per-tool maps.
...
* **`export_modules`**: Export VBA source code modules to disk.
  ...
  - **Sidecar manifest** (Refs #1724, WU-4): after every non-readOnly apply run, Dysflow emits `<destinationRoot>/export-manifest.json` with one per-artifact record `{ moduleName, fileType, relativePath, codec, sha256, byteLength }`. `codec` is BOM-driven (`utf-8` / `utf-16le` / `utf-16be` / `unsupported`); `sha256` covers the raw bytes including the BOM. ReadOnly / plan mode skips emission. Consumers of the destination directory should expect this file alongside the per-module artifacts.
* **`export_all`**: Export all VBA modules (including code-less forms and reports visual layouts) and saved queries from the database.
  ...
  - **Sidecar manifest** (Refs #1724, WU-4): same shape as `export_modules` — `<destinationRoot>/export-manifest.json` with one per-artifact record. `prune` cleans up the sidecar alongside the managed module files because it shares the destination root.
  - **`prune`**: when `true`, after a **fully clean** export, deletes on-disk source files (`.bas`/`.cls`/`.form.txt`/`.report.txt`) whose module no longer exists in the binary, so the destination mirrors the binary. It deletes directly and lists the removed paths under `prune.deleted`. Guarantees:
    ...
    - **A file that cannot be deleted is reported, not thrown.** A locked file (Access or an editor holding a handle) rejects with `EPERM`/`EBUSY`; `force:true` suppresses only `ENOENT`. Such an entry is recorded under `prune.failed` as `{ path, message }` and the sweep continues, so `prune.deleted` stays a complete record of what was actually removed. `prune.failed` is `[]` on a clean sweep; a non-empty `failed` alongside `applied: true` means the destination does not yet fully mirror the binary — release whatever holds the listed paths and re-run.
* **`import_modules`**: Import VBA source modules from disk.
  ...
  - Applied imports preserve caller order in the terminal `DYSFLOW_RESULT`. Multi-module calls retry only failed names after a pass that made progress; a single-module failure or a no-progress pass is terminal. Terminal failures keep top-level `VBA_IMPORT_FAILED` plus typed per-module phase, rollback, lock-owner, fallback, and remediation evidence.
  - Persistence is save-only: new components and re-imported forms/reports request `RunCommand(280)` after a fully successful import. A post-import save failure is returned as a warning while successful module results remain authoritative. Dysflow never compiles VBA; the human compiles in Access before testing. See [VBA import orchestration](../architecture/vba-import-orchestration.md).
...
* **`import_all`**: Bulk import all local modules into the Access project.
  ...
  - Uses the same ordered retry, rollback, save-only, warning, and human-compile contract as `import_modules`; the failure prefix remains `Import-all` in the terminal error message.
...
* **`verify_code`**: The single dry-run tool that compares exported VBA/Form source against the disk tree. It NEVER mutates Access. One tool covers every comparison scope:
  ...
  - The compact default reports `summaryStructured` as `{ matched, actionableTotal, nonActionableTotal }`, actionable `summaryByCategory`, non-actionable `nonActionableByCategory`, `bulkImportable[]`/`bulkExportable[]`, warnings, version/rule fingerprints, and the `hasFunctionalDifferences` / `actionableOk` decision.
  - `nonActionableByCategory` reports `{ caseOnly, whitespaceOnly, attributeOnly, formSerializationOnly, encodingOnly, commentOnly, continuationOnly, statementBoundaryOnly, nonActionableMixed }` — 9 named buckets, the reason `ok` can be false while `actionableOk` is true. A response with `actionableOk:true` and `nonActionableByCategory.whitespaceOnly:3` means three compared artifacts differ only in line endings, indentation, or trailing whitespace and must not be synced. `nonActionableMixed` is the catch-all for diffs that equalize only under a combination of non-actionable normalizers (case + comment, etc.). Both category maps are absent in strict mode, which carries no classification.
  - Both category maps are aggregate counts. They do not identify which module belongs to each category. Never assign a category to a named module by aligning totals with another array or report; use one whole-scope `diagnostic:true` call and read `actionableDifferent[]` and `nonActionableDifferent[]` when membership matters.
  - One logical Access form or report can emit separate `.cls` and `.form.txt` entries for code-behind and layout. They are classified independently and can both contribute to the aggregate counts while sharing the same module name.
  - The compact machine `recommendedAction` (`no_action`, `import_to_binary`, `export_to_src`, or `manual_merge`) gives the sync direction in one shot. The human-readable `recommendation` remains available in diagnostic mode.
  - `ok` remains raw source/binary parity for compatibility and may be false for non-actionable noise. Consumers decide whether to sync from `actionableOk` / `recommendedAction`, never from raw `ok` alone.
  - `diagnostic:true` restores `matched`, missing/different arrays, `diffs`, `actionableDifferent`, `nonActionableDifferent`, `moduleCounts`, `summaryUnits`, per-entry `classification`/`reason`/`recommendation`, and inline snippets. Diagnostic mode implies snippet generation; `diff` remains a compatibility input rather than the response-detail switch.
  - Parameters: `moduleNames` (array, optional), `diff` (boolean, optional), `diagnostic` (boolean, optional — return full raw evidence and snippets), `strict` (boolean, optional — restore byte/text-exact comparison), `timeoutMs` (number, optional), `strictContext` (boolean, optional)
...
| `whitespaceOnly` | Only line endings (CRLF/LF), trailing whitespace, trailing blank lines, or leading indentation. Indentation is folded for code modules (`.bas`/`.cls`/`.frm`) only; structural indentation in a `.form.txt`/`.report.txt` still reaches the functional diff | No |
...
| `caseOnly` | Only identifier/keyword casing differs (`Me.Name` vs `Me.name`). VBA is case-insensitive and the VBE re-cases identifiers project-wide on import. String-literal and comment bodies are compared **case-sensitively**, so a runtime-visible text change is NOT absorbed here. A pair that differs only in indentation is reported as `whitespaceOnly`, never as `caseOnly` (#1669) | No |
...
| `commentOnly` | Only whole-line `'` or `Rem` comment body differs (case, content, or both). The VBE never reads comment bodies at runtime, so a comment-only reflow or content update is non-functional. String literals are untouched by this normalization | No |
| `continuationOnly` | Only VBA line-continuation reflow differs (`<spaces>_<EOL>` joining the next physical line). Source splitting an expression across two physical lines and binary keeping it on one is non-functional under the VBA grammar | No |
| `statementBoundaryOnly` | Only colon-separated vs newline-separated statement boundaries differ (e.g. `Dim a: Dim b` vs two physical lines). The classifier preserves `:=`, date literals, and string-literal colons so the rewrite is always safe | No |
| `nonActionableMixed` | Two or more distinct non-actionable families contributed to equalize the texts (e.g. case in an identifier PLUS case in a comment). The single-family detectors above remain the preferred path for single-family diffs; this bucket is the catch-all for compound diffs | No |
