...
- `fix(run-vba)`: decide the `PROCEDURE_NOT_CALLABLE` remediation from the preflight verdict, not from dots in the name (#1787).
  - Adversarial review corroborated two defects in the first cut of that branch, which sniffed `invokedProcedureName.includes(".")`. Misattribution: `found-elsewhere` and `unverifiable` forward the caller's name verbatim, so a legitimate `referencedDatabase.procedure` call was told to "retry unqualified" — advice that silently retargets the call to a same-named local procedure. And the retry loop this issue exists to kill: the suggested name was cut at the FIRST dot, so a multi-segment name produced another dotted name that re-entered the same branch on retry.
  - The service now passes the preflight's own verdict to the reclassifier. When the prefix was stripped, qualification is ruled out by construction and the stale-p-code remediation stands regardless of what the procedure name looks like. When a qualifier was supplied that the preflight could not tie to a local module, the envelope names it, asks whether it is a referenced database whose reference resolves, and offers a retry name only when that name is terminal.
  - Paths: `src/core/services/vba-service.ts`, `test/core/services/run-vba-qualified-name-1787.test.ts`, `docs/tools/run-vba.md`, `skills/dysflow-usage/SKILL.md`, `skills/dysflow-usage/assets/examples/run-vba.md`, `skills/dysflow-usage/assets/anti-patterns.md`, `skills/dysflow-usage/references/error-codes.md`.
...
- `feat(pi)`: ship `@aroman22/dysflow-pi` as a standalone Pi package that exposes Dysflow tools with custom rendering without modifying `gentle-pi` (Refs #1723). Native registration via `pi.registerTool({ name, renderShell: 'self', renderCall(), renderResult() })`; chrome glyph renders `⚡ Dysflow · <operationLabel>` (no args, secrets, or paths); MCP compatibility keeps `directTools:false` on the `dysflow` server for diagnostics; version is pinned to the root package; `dysflow install` / `update` / `uninstall` wire the npm package ownership-safely. Zero edits to `gentle-pi`. 25-test TDD suite across rendering, packaging, install/uninstall, and quality gates passes locally. OpenSpec SDD artifacts at `openspec/changes/dysflow-pi-branding/`. Clean-Pi visual acceptance remains a human gate.
...
- fix(resolver): `resolve_project` anchors an ambiguous sibling-worktree fleet to the requested `cwd`. When exactly one discovered project is rooted at that `cwd`, it resolves instead of returning `ambiguous` — a `worktree-per-change` fleet where every sibling commits the same `id` could not be disambiguated by the recovery envelope, because choosing the shared id narrowed nothing. A `cwd` that owns no `.dysflow/project.json` is still ambiguous and still returns the full envelope. See `docs/architecture/worktree-fleet-project-resolution.md` (#1668).
- fix(resolver): a rejected recovery trio names the field at fault. `error.missingParam` reports an omitted `projectId` / `projectChoiceReason` / `recoveryToken`; `error.rejectedFlag` reports one supplied with an unusable value (wrong literal reason, unknown or consumed token, out-of-envelope `cwd` or `projectId`). The previous single "must include the exact trio" message covered six distinct causes (#1668).
- fix(mcp): a schema rejection reports an unknown parameter before a missing required one. `lint_module({ moduleName })` now returns `moduleName is not allowed. … Did you mean 'module'?` instead of `module is required.`, which sent callers looking for a second parameter to add (#1668).
- fix(mcp): `list_procedures`, `get_procedure`, `find_references`, `detect_dead_code`, `validate_manifest`, and `lint_module` route schema rejections through the shared enrichment, so their `MCP_INPUT_INVALID` envelopes carry `missingParam` / `rejectedFlag` like every dispatch-gated tool (#1668).
- fix(bootstrap): the snapshot adds `humanCompilePendingScope` (`"project-in-scope"` | `"no-project-in-scope"`). `bootstrap` never resolves a project, so a bare `humanCompilePending: false` was indistinguishable from "no project was in scope" and read as a readiness verdict on a worktree the resolver could not target (#1668).
...
- fix(dysflow-vba-manager): export plan mode (`apply:false` / `dryRun:true`) no longer crashes with `VBA_MANAGER_UNEXPECTED_EXIT` — the #1055 fix routed plan exports through the runner as `-ReadOnly` but `dysflow-vba-manager.ps1` never declared the parameter. The script now accepts `[switch]$ReadOnly`, resolves export targets without writing anything (no module files, no directories, no query export), and emits `DYSFLOW_RESULT { ok, readOnly: true, planned[] }`. Caught by the release-gate E2E battery (unit tests pinned the contract at the mocked executor seam); guarded by new Pester behavioral tests + a cheap source-text parity pin (#1063)
