...
describe("VbaSyncLegacyService", () => {
  ...
  it("returns VBA_NO_TESTS_SELECTED without calling PowerShell when proceduresJson is empty (#211)", async () => {
    ...
    await expect(service.execute("test_vba", { proceduresJson: "[]" })).resolves.toMatchObject({
      ok: false,
      error: { code: "VBA_NO_TESTS_SELECTED" },
    });
    expect(calls).toEqual([]);
  ...
  it("runs compile before test_vba plan execution when compile is requested", async () => {
    ...
    const service = new VbaSyncLegacyService({
      executor: async (request) => {
        ...
        return {
          exitCode: 0,
          stdout: request.action === "Compile" ? '{"ok":true}' : '[{"ok":true}]',
          stderr: "",
          durationMs: 4,
          timedOut: false,
        };
    ...
    await expect(service.execute("test_vba", { compile: true })).resolves.toMatchObject({
      ok: true,
    });
  ...
  it("verify_code exports to a temporary directory and compares without overwriting source", async () => {
    ...
    expect(calls).toEqual([
      expect.objectContaining({ action: "Export", destinationRoot: expect.any(String) }),
    ]);
  });
  ...
  it("returns VBA_TESTS_FAILED when any test result has ok: false (#273)", async () => {
    const service = new VbaSyncLegacyService({
      executor: async () => ({
        exitCode: 0,
        stdout:
          '[{"ok":true,"procedure":"Test_A"},{"ok":false,"procedure":"Test_B","error":"Assert failed"}]',
        stderr: "",
        durationMs: 3,
        timedOut: false,
      }),
