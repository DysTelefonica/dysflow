...
class FakeDiagnosticsService {
	public requests: unknown[] = [];
	constructor(
		private readonly result: OperationResult<AccessDiagnosticsResult>,
	) {}
	async run(
		request: unknown,
...
describe("MCP tool registration over core services", () => {
	it("registers protocol-safe MCP tools that invoke the matching core services", async () => {
		const vba = new FakeVbaService(
			successResult({ returnValue: "refreshed" }, { durationMs: 7 }),
		);
		...
		expect(
			tools.find((tool) => tool.name === "dysflow.vba.execute")?.inputSchema,
		).toMatchObject({
			type: "object",
		...
		await expect(
			...
			content: [
				{ type: "text", text: JSON.stringify({ returnValue: "refreshed" }) },
			],
			isError: false,
		});
		await expect(
			...
			content: [
				{
					type: "text",
					text: JSON.stringify({ rows: [{ id: 1, name: "Ada" }] }),
				},
			],
			isError: false,
		});
		await expect(
			tools[2]?.handler({ includeEnvironment: true }),
		).resolves.toEqual({
			content: [
				{
					type: "text",
	...
	it("accepts contextId/projectId on short core calls without requiring local path injection", async () => {
		...
		await expect(
			tools
				.find((tool) => tool.name === "dysflow.doctor")
				?.handler({ contextId: "00-no-conformidades-staging-clean" }),
		).resolves.toMatchObject({ isError: false });
		await expect(
			tools
				.find((tool) => tool.name === "dysflow.vba.execute")
				?.handler({
		...
		await expect(
			tools
				.find((tool) => tool.name === "dysflow.query.execute")
				?.handler({
					contextId: "00-no-conformidades-staging-clean",
					sql: "SELECT 1",
					mode: "read",
	...
	it("rejects invalid MCP inputs before calling core services", async () => {
		...
		await expect(
			...
			content: [
				{ type: "text", text: "MCP_INPUT_INVALID: procedureName is required." },
			],
			isError: true,
		});
		await expect(
			...
			content: [
				{ type: "text", text: "MCP_INPUT_INVALID: sql must be a string." },
			],
			isError: true,
		});
		await expect(
		...
		expect(
			tools.find((tool) => tool.name === "catalog_add_control")?.inputSchema
				?.properties,
	...
	it("rejects invalid nested MCP inputs before calling core services", async () => {
		...
		await expect(
			...
			content: [
				{
					type: "text",
					text: "MCP_INPUT_INVALID: mode must be one of: read, write.",
				},
			],
			isError: true,
		});
		await expect(
			...
			content: [
				{
					type: "text",
					text: "MCP_INPUT_INVALID: allowTables[1] must be a string.",
				},
			],
			isError: true,
		});
		await expect(
			...
			content: [
				{
					type: "text",
					text: "MCP_INPUT_INVALID: queryDefinitions[0].sql must be a string.",
				},
			],
			isError: true,
		});
	...
	it("handles legacy run_vba argsJson as MCP input instead of raw JSON-RPC failures", async () => {
		...
		await expect(
			runVba?.handler({ procedureName: "Blank", argsJson: "   " }),
		).resolves.toEqual({
		...
		await expect(
			runVba?.handler({ procedureName: "Single", argsJson: "42" }),
		).resolves.toEqual({
			content: [{ type: "text", text: JSON.stringify({ returnValue: "ok" }) }],
			isError: false,
		});
	...
	it("declares explicit JSON schemas for every MCP tool", () => {
		...
		for (const tool of tools) {
			expect(
				tool.inputSchema,
				`${tool.name} should declare inputSchema`,
			).toMatchObject({ type: "object", properties: expect.any(Object) });
