...
describe("DAO MCP target contracts", () => {
  it.each(["list_tables", "query_sql", "get_relationships"] as const)(
    "%s accepts explicit frontend/backend roles but rejects auto without a table identity",
    (toolName) => {
  ...
  it.each(["get_schema", "count_rows", "distinct_values"] as const)(
    "%s exposes auto because its table identity can drive cross-database lookup",
    (toolName) => {
      const schema = QUERY_TOOL_SCHEMAS[toolName];
      expect(
  ...
  it.each(["list_tables", "get_schema", "count_rows", "distinct_values"] as const)(
    "%s keeps the shared explicit path aliases",
    (toolName) => {
