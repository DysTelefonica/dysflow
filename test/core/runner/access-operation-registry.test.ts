...
describe("Access operation registry and cleanup safety", () => {
	...
	it("purges stale failed and timed_out records while keeping recent failures (issue #227)", async () => {
		const root = await mkdtemp(join(tmpdir(), "dysflow-ops-stale-failures-"));
		const registryPath = join(root, ".dysflow", "runtime", "operations.json");
		try {
			const registry = new FileAccessOperationRegistry({
				filePath: registryPath,
				staleFailurePurgeMs: 1_000,
			});
			await registry.create({
				...base,
				operationId: "op-failed-old",
				status: "failed",
				accessPid: 1201,
				processStartTime: "2026-05-15T10:00:00.000Z",
				updatedAt: "2026-05-15T10:00:00.000Z",
			...
			await registry.create({
				...base,
				operationId: "op-failed-recent",
				status: "failed",
				accessPid: 1203,
				processStartTime: "2026-05-15T10:00:01.000Z",
				updatedAt: new Date().toISOString(),
			});

			const recent = await registry.listRecent({ limit: 10 });
			expect(recent.map((entry) => entry.operationId)).toContain("op-failed-recent");
			expect(recent.map((entry) => entry.operationId)).not.toContain("op-failed-old");
			expect(recent.map((entry) => entry.operationId)).not.toContain("op-timeout-old");
		} finally {
			await rm(root, { recursive: true, force: true });
		}
	});

	it("preserves original rename/write failure when temp-file cleanup also fails (issue #231)", async () => {
		const root = await mkdtemp(join(tmpdir(), "dysflow-ops-temp-cleanup-fail-"));
		const registryPath = join(root, ".dysflow", "runtime", "operations.json");
		try {
			vi.resetModules();
			vi.doMock("node:fs/promises", async () => {
			...
			const { FileAccessOperationRegistry: TempFailureRegistry } =
				await import("../../../src/core/operations/access-operation-registry.js");
			const registry = new TempFailureRegistry({ filePath: registryPath });

			await expect(
				registry.create({
					...base,
					operationId: "op-rename-failure",
					status: "starting",
					accessPid: null,
					processStartTime: null,
					updatedAt: "2026-05-15T10:00:00.000Z",
	...
	it("uses finite stale failure purge default so old failed records are evicted in production-like setup (issue #227)", async () => {
		const root = await mkdtemp(join(tmpdir(), "dysflow-ops-default-stale-purge-"));
		const registryPath = join(root, ".dysflow", "runtime", "operations.json");
		try {
			...
			const recent = await registry.listRecent({ limit: 10 });
			expect(recent.map((entry) => entry.operationId)).not.toContain("op-old-failed");
		} finally {
			await rm(root, { recursive: true, force: true });
		}
	});

	it("fails write when registry post-rename integrity verification detects mismatched payload (issue #231)", async () => {
		const root = await mkdtemp(join(tmpdir(), "dysflow-ops-integrity-mismatch-"));
		const registryPath = join(root, ".dysflow", "runtime", "operations.json");
		try {
			vi.resetModules();
			vi.doMock("node:fs/promises", async () => {
				...
				return {
					...
					readFile: async (
						...
						if (String(path).endsWith("operations.json")) {
							return "{\"records\":[]}";
						}
			...
			const { FileAccessOperationRegistry: IntegrityRegistry } =
				await import("../../../src/core/operations/access-operation-registry.js");
			const registry = new IntegrityRegistry({ filePath: registryPath });

			await expect(
				registry.create({
					...base,
					operationId: "op-integrity-failure",
					status: "starting",
					accessPid: null,
					processStartTime: null,
					updatedAt: "2026-05-15T10:00:00.000Z",
	...
	it("propagates lock acquire IO errors without masking as timeout (issue #233)", async () => {
		const root = await mkdtemp(join(tmpdir(), "dysflow-ops-lock-io-error-"));
		const registryPath = join(root, ".dysflow", "runtime", "operations.json");
		const lockPath = `${registryPath}.lock`;
		try {
			vi.resetModules();
			vi.doMock("node:fs/promises", async () => {
				...
				return {
					...
					mkdir: async (
						...
						if (path === lockPath) {
							throw new Error("EACCES lock path");
						}
						return actual.mkdir(path, options);
					},
				};
			});
			const { FileAccessOperationRegistry: LockIoRegistry } =
				await import("../../../src/core/operations/access-operation-registry.js");
			const registry = new LockIoRegistry({ filePath: registryPath, lockTimeoutMs: 25 });

			await expect(
				registry.create({
					...base,
					operationId: "op-lock-io-error",
					status: "starting",
					accessPid: null,
					processStartTime: null,
					updatedAt: "2026-05-15T10:00:00.000Z",
				}),
			).rejects.toThrow(/EACCES lock path/);
		} finally {
			vi.doUnmock("node:fs/promises");
			vi.resetModules();
			await rm(root, { recursive: true, force: true });
