...
describe("VbaFormService", () => {
	...
	it("can catalog control and update forms/catalog.json", async () => {
		...
		expect(catalogContent.forms["Form_Cataloged"]).toEqual([
			{ name: "txtUser", type: "TextBox" }
		]);
	});
	...
	it("can harvest forms and reports from forms and reports directory", async () => {
		...
		expect(result).toMatchObject({
			...
			data: {
				destinationRoot: root,
				forms: [
					{ name: "MyForm", kind: "Form", controls: 2 }
				],
				reports: [
					{ name: "MyReport", kind: "Report", controls: 3 }
				],
				total: 2
			}
		});
