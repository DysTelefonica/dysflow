...
describe("runtime marker — persistent runtime dir across Windows users", () => {
	...
	it(
		"install with --runtime-dir writes marker at the system marker path",
		async () => {
			...
			const result = await handleInstallCommand(
				["--runtime-dir", runtimeDir, "--no-tui"],
				{ env, packageRoot: pkgRoot },
			);
	...
	it(
		"update without --runtime-dir or DYSFLOW_HOME reuses the marker-persisted runtime dir",
		async () => {
	...
	it(
		"DYSFLOW_HOME takes precedence over marker",
		async () => {
	...
	it(
		"--runtime-dir takes precedence over both DYSFLOW_HOME and marker",
		async () => {
			...
			const result = await handleInstallCommand(
				["--runtime-dir", explicitRuntime, "--no-tui"],
				{ env, packageRoot: pkgRoot },
			);
	...
	it(
		"missing/unreadable marker falls back to LOCALAPPDATA\\dysflow",
		async () => {
	...
	it(
		"up-to-date update still writes marker so future calls reuse the same runtime",
		async () => {
			...
			const result = await handleUpdateCommand(
				...
				{ env, releaseUpdateProvider: markerProvider },
			);
