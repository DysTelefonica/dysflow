...
describe("CLI --help consistency per subcommand (#591)", () => {
  describe("mcp --help", () => {
    it.each([["--help"], ["-h"]])(
      "exits 0 with usage on stdout and no side effects for %s",
      async (flag) => {
  ...
  describe("doctor --help", () => {
    it.each([["--help"], ["-h"]])(
      "exits 0 with usage on stdout and does NOT run diagnostics for %s",
      async (flag) => {
  ...
  describe("access --help", () => {
    it.each([["--help"], ["-h"]])(
      "exits 0 with usage on stdout and does NOT instantiate a runner for %s",
      async (flag) => {
