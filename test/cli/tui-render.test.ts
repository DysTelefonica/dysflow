...
describe("Dysflow TUI rendering", () => {
  ...
  it("shows concise update guidance when local is outdated", () => {
    ...
    expect(output).toContain("pnpm add -g git+https://github.com/DysTelefonica/dysflow.git#v0.2.1");
  });
  ...
  it("resolves latest version through an injectable provider", async () => {
    const status = await buildVersionStatus.fromProvider("0.2.0", async () => "v0.2.1");
