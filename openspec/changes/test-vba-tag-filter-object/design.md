...
1. **Schema layer (`src/shared/validation/schema-props.ts:169`)**: refresh the
   `filter` description so the live `describe_tool({name:"test_vba"})` advertises
   both shapes. Keep `type: "string"` — see Decision D1.
...
| File | Action | Why |
|---|---|---|
| `src/shared/validation/schema-props.ts:169` | Modify | Refresh `SCHEMA_PROPS.filter` description to advertise both shapes; type stays `"string"`. |
| `src/adapters/mcp/schemas/vba-sync-schemas.ts:541` | Modify (auto) | Inherits updated `SCHEMA_PROPS.filter`. |
...
| `test/adapters/vba-sync/vba-execution-adapter.test.ts` | Modify | Add `describe("parseTestFilter object branch")` and `describe("matchesTestFilter tag_only branch")` (see §6). |
...
### D1 site — `SCHEMA_PROPS.filter`

```ts
filter: {
  type: "string",                                                 // type unchanged (validator constraint — D1)
  description:
