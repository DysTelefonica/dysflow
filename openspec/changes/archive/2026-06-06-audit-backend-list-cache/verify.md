...
- `git log --oneline --decorate -12` shows `3c4692f` as `HEAD -> staging`; the later compile-fix commit does not revert behavior and only moves DAO transaction calls from `Database` to `Workspace`.
