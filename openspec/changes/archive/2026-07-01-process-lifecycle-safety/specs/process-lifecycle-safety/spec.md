# Delta for process-lifecycle-safety
...
## F2 Open Question — Design Decision Recorded

The proposal flagged `force: true` interpretation as an open question
(two possible readings: "fail-safe that never bypasses the running gate"
vs. "kill at all costs even mid-flight"). This spec confirms the first
interpretation as the design decision for issue #620:

- `force: true` MUST remain the safe-by-default path. It already allows
  retired PIDs, terminal-state records, and `pid_unknown` records to be
  cleaned; it MUST NOT allow killing an in-progress operation.
- A new typed error `CLEANUP_RUNNING_FORCE_REFUSED` is added so callers
  can disambiguate "running under force" from `CLEANUP_STATUS_NOT_ELIGIBLE`
  (which still fires for non-running ineligible statuses under
  `force:false`).
- If a future cycle requires "kill mid-flight at all costs", introduce a
  separate `hardKill: true` flag in a follow-up change rather than
  reusing `force`. That keeps the safety contract readable.

If the user instead wants the second interpretation (`force: true` means
"kill no matter what"), surface that as a spec reshape before sdd-design
begins — every requirement and scenario in this delta that mentions
`CLEANUP_RUNNING_FORCE_REFUSED` would need to flip, and the F2 PR
becomes a behavior change rather than a contract tightening.
