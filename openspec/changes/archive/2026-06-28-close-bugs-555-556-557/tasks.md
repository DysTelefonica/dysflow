...
| F — incomplete managed-subfolder source discovery | `pnpm vitest run test/adapters/vba-sync/vba-modules-adapter.test.ts --testNamePattern "managed subfolder cannot be read"` failed before implementation because a readable `modules/Live.bas` plus unreadable `forms/` still allowed prune to continue. | `discoverManagedSource` now distinguishes missing managed folders from unreadable existing folders for prune: missing folders remain compatible, unreadable managed folders fail with `IMPORT_PRUNE_SOURCE_UNSAFE` before `list_objects`/`delete_module`. `pnpm test`; `pnpm build`; `pnpm lint`; `Invoke-Pester scripts/tests/`; CI `28338833804`. | Fixed |
