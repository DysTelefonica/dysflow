# ai-form-ui-builder Specification

## Purpose

Add an AI-first workflow for designing and validating Microsoft Access form UIs so contributors can analyze an existing form, map behavior, generate/apply a design plan, copy reference UI patterns, and verify the result without hand-waving. The workflow is protocol-neutral in `src/core/**` and adapts Access/VBA-specific extraction in `src/adapters/vba-sync/**`, keeping the dependency direction consistent with the rest of the OpenSpec.
