...
2. **The live runtime is the source of truth.** `get_capabilities` and `codegraph upgrade --check` are
...
4. **Every runtime claim must be verifiable today** via `get_capabilities`; if not, remove it.
...
5. **Never introduce a tool name not in the live `toolsVisible`.**
