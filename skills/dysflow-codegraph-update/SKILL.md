...
description: "Trigger: Dysflow release, CodeGraph release, runtime drift, consumer-skill drift, failed pointer rollout. Aligns owned skills and agent pointers with bootstrap-first candidate evidence, including a schema-derived audit of personal consumers."
license: Apache-2.0
metadata:
  author: "Andrés Román"
  version: "3.2.0"
  ...
  last_codegraph_vba_version: "2.0.0"
...
- **HR-1 — Resolve ownership before editing.** The Dysflow release bundle is canonical for exactly six Dysflow-owned runtime skills:
...
- **HR-2 — Trust the candidate runtime, not memory.** The authoritative chain is
...
- **HR-3 — No bitácoras in skills.** Delete changelog-only sections; do not duplicate the CHANGELOG.
- **HR-4 — Replace literal tool names only through exact edits.** Never use a free rewrite for a contract rename.
- **HR-5 — Never introduce a tool name absent from live schema index.** Use
   `advertised` only for claims about `tools/list`; callable dispatch is wider.
- **HR-6 — No mechanical version-floor bump.** Raise a `requires:` floor only if the changelog confirms
   the new version did not reintroduce the original bug.
- **HR-7 — Use `apply:false` as the valid non-executing plan** for `run_vba` / `test_vba` when `allowedProcedures`
   is empty — a documented opt-out, not a bug workaround.
- **HR-8 — Keep the AGENTS.md core and `rules/*.md` Dysflow/CodeGraph sections in scope.** Keep the core lean;
...
- **HR-9 — Keep one authoritative place per runtime rule.** Use `dysflow-usage` for names/flags/errors,
   `rules/dysflow-codegraph.md` for operation, the AGENTS.md core as the short pointer — never
   duplicated with drift.
- **HR-10 — Run the example verifier when runtime drift changes `dysflow-usage`.** Execute
    `../dysflow-usage/assets/scripts/verify-examples-vs-runtime.ps1` and confirm it exits 0 before finishing.
- **HR-11 — After editing skills, use ownership-aware verification.** Verify release-owned destinations
...
- **HR-12 — Deprecate instead of rewriting a moved live surface.** For a skill whose surface moved to dysflow,
    follow `references/deprecated-skills.md` (banner + migration table; git keeps the old body).
- **HR-13 — Never edit, compact, or restructure a skill you do not own.** Establish Dysflow ownership
...
- **HR-14 — Use progressive candidate introspection, not `get_capabilities` alone.** A
...
- **HR-15 — Fail closed on semantic drift.** The semantic audit distinguishes DRIFT (docs/consumer
...
- **HR-16 — Run a consumer-skill semantic audit for every Dysflow or CodeGraph release.** Enumerate the
    ...
    guidance against the same candidate captures used for the release bundle, and CodeGraph
    index-directory claims against the `indexPath` from `codegraph status --json`. A grep hit list is
...
| Condition | Action |
|---|---|
| Personal canonical root is unavailable | Report that ownership lane as unavailable; never infer from installed mirrors. |
| Personal repository is dirty | Complete the read-only audit, but do not edit or commit that lane; report exact blocked files. |
| Active consumer guidance contradicts candidate evidence | Classify it as DRIFT and update only its canonical owner after the dirty-tree gate passes. |
| A historical example uses an obsolete contract | Classify it separately; keep history out of active guidance and add a current equivalent only when users can copy the example. |
| Candidate introspection contradicts itself | Report RUNTIME CONTRACT GAP and stop documentation repair for that claim. |
...
`bootstrap`/introspection evidence → run the consumer-skill semantic audit → update only canonical sources → run each owner's validation
...
|---|---|---|
| `runtime_state` | object | Literal Dysflow and CodeGraph versions, write gates, policy, and callable/advertised counts. |
| `ownership_lanes` | object[] | Canonical roots, owners, availability, and dirty-state blockers. |
| `changed_files` | object[] | Owner, path, SHA256 before/after, and line delta. |
| `diff_summary` | object[] | Per-change classification: deleted history, corrected contract, or renamed surface. |
| `propagation` | object[] | Owner-controlled propagation and recursive destination hash evidence. |
| `instruction_surfaces` | object[] | AGENTS/rules sections touched and lean-core line counts. |
| `semantic_audits` | object | Separate release DRIFT, active consumer DRIFT, historical-only, dirty blockers, and runtime gaps. |
| `open_decisions` | string[] | Questions requiring the user; empty when none. |
| `smoke_test` | object | Commands, exit status, and result. |
| `runtime_snapshot` | object | Engram save status and topic keys. |

## §6 Arnés regeneration (ARN)
...
| Symptom | Fix |
|---|---|
| Treating grep hits as proof that consumers are aligned | Validate tool, parameter, enum, result, migration, and config claims against candidate captures. |
| Auditing installed mirrors as canonical input | Resolve the owning release or personal repository first. |
| Mixing historical examples with active DRIFT | Report separate arrays and require current equivalents for executable examples. |
| Editing a dirty personal lane during release maintenance | Audit read-only, report blockers, and wait for a clean owner-controlled change. |
