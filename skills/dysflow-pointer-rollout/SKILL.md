...
12. **Discover the dysflow target, do not hardcode it.** The 9th target is the unique `main` worktree from the Dysflow Git worktree list; the current resolved path is `C:\00repos\codigo\dysflow\00_main\AGENTS.md`. Discovery failure is a hard error. Never fall back to the legacy project path.
