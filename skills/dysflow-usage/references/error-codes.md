...
> **Source of truth**: candidate-runtime full schema and selective `describe_tool({name:"<X>"}).errorCodes` captures verified for the v4.3.0 release on 2026-09-02. The runtime is authoritative.
>
...
| `PROCEDURE_NOT_CALLABLE` | VBA runtime | `apply` failed because Access refused to invoke the procedure despite it being present in `VBComponents`. Stale p-code. Recognized from Access's own "cannot run the macro" / "can't find the procedure" / "object closed or doesn't exist" messages, in either localization. | Same as above: force a manual recompile, retry. |
