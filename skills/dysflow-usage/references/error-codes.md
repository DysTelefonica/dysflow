...
| `PROCEDURE_NOT_CALLABLE` | VBA runtime | `apply` failed because Access refused to invoke the procedure despite it being present in `VBComponents`. Recognized from Access's own "cannot run the macro" / "can't find the procedure" / "object closed or doesn't exist" messages, in either localization. | Read `error.remediation`. When `error.details.invokedProcedureName` still carries a `.`, Access could not resolve that qualifier at all (#1787) — retry with the bare procedure name. Only when the invoked name was already unqualified is stale p-code the cause; then force a manual recompile and retry. |
