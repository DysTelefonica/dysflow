...
| Friction | Use | Read before acting |
|---|---|---|
| Need to create a config, optionally with a procedure allowlist, or inherit one from a sibling worktree | [`setup_project`](../assets/examples/setup-project.md) | Preview `resolvedConfig`. A fresh project needs NO allowlist: the MCP procedure gate is default-allow and the write gate is authoritative. Add `capabilities.procedures.allow` plus `capabilities.procedures.strictMode: true` only when the project actually wants enforcement (CI, fleet automation, a shared runner); use `fromCwd` + `overrideProjectRoot` for a sibling import |
