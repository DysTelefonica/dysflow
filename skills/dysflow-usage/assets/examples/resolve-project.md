...
`json
{
  ...
  "arguments": {
    /* TODO: populate per live describe_tool({name:"resolve_project"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"resolve_project"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"resolve_project"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"resolve_project"})`.
