...
> **Phase**: recovery  Â·  **Access**: conditional-write  Â·  **Status**: preferred (_meta["dysflow/workflow"].status)

## What it does

Retire one Dysflow-owned operation through ownership-safe cleanup.
...
`json
{
  ...
  "arguments": {
    /* TODO: populate per live describe_tool({name:"cleanup_access_operation"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"cleanup_access_operation"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"cleanup_access_operation"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"cleanup_access_operation"})`.
