...
> **Phase**: forms  Â·  **Access**: conditional-write  Â·  **Status**: preferred (_meta["dysflow/workflow"].status)

## What it does

Distribute several controls evenly in one geometry operation.
...
`json
{
  ...
  "arguments": {
    /* TODO: populate per live describe_tool({name:"form_distribute_controls"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"form_distribute_controls"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"form_distribute_controls"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"form_distribute_controls"})`.
