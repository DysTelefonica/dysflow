...
> **Phase**: recovery  Â·  **Access**: read-only  Â·  **Status**: preferred (_meta["dysflow/workflow"].status)

## What it does

Inspect the filtered operation timeline after diagnose identifies a failure.
...
`json
{
  "name": "logs",
  "arguments": {
    /* TODO: populate per live describe_tool({name:"logs"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"logs"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"logs"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"logs"})`.
