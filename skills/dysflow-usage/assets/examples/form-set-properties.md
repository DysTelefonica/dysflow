...
Update several properties on one control atomically.
...
`json
{
  ...
  "arguments": {
    /* TODO: populate per live describe_tool({name:"form_set_properties"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"form_set_properties"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"form_set_properties"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"form_set_properties"})`.
