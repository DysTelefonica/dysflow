...
Apply a validated form design plan through the guarded write seam.
...
`json
{
  ...
  "arguments": {
    /* TODO: populate per live describe_tool({name:"apply_form_design_plan"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"apply_form_design_plan"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"apply_form_design_plan"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"apply_form_design_plan"})`.
