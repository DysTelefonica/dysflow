...
`json
{
  "name": "schema",
  "arguments": {
    /* TODO: populate per live describe_tool({name:"schema"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"schema"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"schema"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"schema"})`.
