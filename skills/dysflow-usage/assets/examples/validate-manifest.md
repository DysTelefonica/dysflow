...
> **Phase**: tests  Â·  **Access**: read-only  Â·  **Status**: preferred (_meta["dysflow/workflow"].status)

## What it does

Validate tests.vba.json procedure references before invoking test_vba.
...
`json
{
  ...
  "arguments": {
    /* TODO: populate per live describe_tool({name:"validate_manifest"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"validate_manifest"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"validate_manifest"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"validate_manifest"})`.
