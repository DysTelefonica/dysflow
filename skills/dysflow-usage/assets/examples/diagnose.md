...
Start recovery with one aggregated project health snapshot.
...
`json
{
  "name": "diagnose",
  "arguments": {
    /* TODO: populate per live describe_tool({name:"diagnose"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"diagnose"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"diagnose"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"diagnose"})`.
