...
Inspect one selected tool's complete contract after compact discovery.
...
`json
{
  "name": "describe_tool",
  "arguments": {
    /* TODO: populate per live describe_tool({name:"describe_tool"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"describe_tool"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"describe_tool"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"describe_tool"})`.
