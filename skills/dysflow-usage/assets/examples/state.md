...
Inspect operation, marker, lock, and counter state during recovery.
...
`json
{
  "name": "state",
  "arguments": {
    /* TODO: populate per live describe_tool({name:"state"}) output. */
...
`json
{
  ...
  "isError": false,
  "...": "see describe_tool({name:"state"}) for the live result contract"
}
`
...
- Live schema: `schema({view:"full"})` or `describe_tool({name:"state"})`.
...
- Other tool-specific runtime values per `describe_tool({name:"state"})`.
