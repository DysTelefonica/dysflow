...
8. **`run_vba` plan/apply agreement (#1174)** — `run_vba` parses `procedureName` into `<module>.<procedure>` once and threads the parsed `moduleName` + `procName` through both the `apply:false` plan and the `apply:true` preflight. The two paths therefore MUST agree on procedure resolution for the same input. If you observe a divergence:
   ...
   - `apply: true` fails with `PROCEDURE_NOT_CALLABLE` → the procedure is present in the binary's `VBComponents` but Access refused to invoke it. Read `error.remediation`: since #1787 it names the suspect from `error.details.invokedProcedureName`. A name that still carries a `.` was never resolvable — Access resolves `Application.Run` by procedure name, and only a REFERENCED database's project name qualifies — so retry unqualified. Only an unqualified name that still fails points at stale p-code; then recompile in Access VBE, ONCE. If a `Debug → Compile` that reported no errors does not change the outcome, stop recompiling and report it — that loop cannot terminate.
