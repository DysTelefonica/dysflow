...
export class AccessOperationCleanupService {
  ...
  async cleanup(request: { operationId: string; accessPath: string; force?: boolean }): Promise<OperationResult<AccessCleanupResult>> {
    ...
    if (record.accessPid === null || record.processStartTime === null || record.status === "pid_unknown" || record.status === "running_untracked") {
      return failureResult(createDysflowError("CLEANUP_PID_UNKNOWN", "Cleanup refused because the operation has no owned Access PID."));
    }

    if (!request.force && !ELIGIBLE_STATUSES.has(record.status)) {
      return failureResult(createDysflowError("CLEANUP_STATUS_NOT_ELIGIBLE", `Cleanup refused for operation status ${record.status}.`));
    }

    const process = await this.options.processInspector.getProcess(record.accessPid);
    if (process === undefined) {
      return failureResult(createDysflowError("CLEANUP_PROCESS_NOT_FOUND", `Process ${record.accessPid} no longer exists.`));
    }

    if (process.name.toUpperCase() !== "MSACCESS.EXE") {
      return failureResult(createDysflowError("CLEANUP_PROCESS_NAME_MISMATCH", `Cleanup refused because PID ${record.accessPid} is ${process.name}.`));
    }

    if (process.startTime !== record.processStartTime) {
      return failureResult(createDysflowError("CLEANUP_PROCESS_START_TIME_MISMATCH", "Cleanup refused because processStartTime differs from the registry."));
    }

    const commandLine = process.commandLine ?? record.commandLine ?? "";
    if (commandLine.length > 0 && !commandLine.toLowerCase().includes(record.accessPath.toLowerCase())) {
