...
import { stringValue, sanitizeSecrets, truthy } from "../utils/index.js";
...
export async function compareSourceAgainstBinary(
	...
	try {
		...
		const preflightDiagnostics = diagnosticsFromPreflightCleanup(await ctx.runPreflightCleanup(target.data));
		...
		return successResult({ operation: toolName, ...comparison }, { diagnostics: preflightDiagnostics, durationMs: result.durationMs });
	} finally {
...
export async function compareVbaSourceTrees(sourceRoot: string, binaryExportRoot: string, moduleNames: readonly string[], includeDiffs: boolean): Promise<Omit<VbaVerifyResult, "operation">> {
...
export async function collectVbaSourceFiles(root: string, moduleFilter: ReadonlySet<string>): Promise<VbaSourceComparisonFile[]> {
