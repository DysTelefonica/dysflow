...
/**
 * True when `targetPath` is the dysflow production runtime directory or lives
 * inside it. Used to refuse write operations that would mutate the installed
 * runtime (AGENTS.md hard rule), e.g. a writes-enabled caller pointing
 * destinationRoot at `%LOCALAPPDATA%\\dysflow`. Resolution mirrors
 * resolveRuntimeDir, so DYSFLOW_HOME / the system marker / the LOCALAPPDATA
 * default are all honored.
 */
export function isWithinRuntime(targetPath: string, env: NodeJS.ProcessEnv): boolean {
  const runtime = normalizePath(resolveRuntimeDir(undefined, env));
  if (!runtime) return false;
  const target = normalizePath(targetPath);
  return target === runtime || target.startsWith(`${runtime}/`);
}
