...
' ============================================
' PUNTO ÚNICO DE LECTURA DE CONFIGURACIÓN DE CACHÉ
' ============================================
' Lee del singleton m_ObjEntorno.CacheHabilitada (TbConfiguracion WHERE ID=1)
' Si falla, retorna False por seguridad.
...
' ============================================
' GET LISTADO FILTRADO SQL (CORAZÓN DEL FILTRADO)
' ============================================
...
Public Function GetListadoFiltradoSQL( _
    ...
    If Not SincronizarCache(p_Error) Then
        If p_Error <> "" Then Err.Raise 1000
        Set GetListadoFiltradoSQL = Nothing
        Exit Function
    End If
    
    SQL = "SELECT * FROM TbCacheListadoNC WHERE 1=1"
    
    If p_Codigo <> "" Then
        SQL = SQL & " AND CodigoNoConformidad = '" & EscapeSQL(p_Codigo) & "'"
    End If
    ...
    If p_Estado <> "" Then
        SQL = SQL & " AND Estado = '" & EscapeSQL(p_Estado) & "'"
    End If
    
    If p_Descripcion <> "" Then
        SQL = SQL & " AND Descripcion LIKE '*" & EscapeSQL(p_Descripcion) & "*'"
    End If
    
    If p_Notas <> "" Then
        SQL = SQL & " AND Notas LIKE '*" & EscapeSQL(p_Notas) & "*'"
    End If
    
    If p_RequiereCE <> "" Then
        SQL = SQL & " AND RequiereControlEficacia = '" & EscapeSQL(p_RequiereCE) & "'"
    End If
    
    If p_ControlEficacia = "Sí" Then
        SQL = SQL & " AND ControlEficacia <> ''"
    ElseIf p_ControlEficacia = "No" Then
        SQL = SQL & " AND ControlEficacia = ''"
    End If
    
    If p_ResponsableCalidad <> "" Then
        SQL = SQL & " AND Responsabilidad = '" & EscapeSQL(p_ResponsableCalidad) & "'"
    End If
    ...
    If p_ResponsableTelefonica <> "" Then
        SQL = SQL & " AND ResponsableTelefonica = '" & EscapeSQL(p_ResponsableTelefonica) & "'"
    End If
    
    If p_Google <> "" Then
        SQL = SQL & " AND (Descripcion LIKE '*" & EscapeSQL(p_Google) & "*' OR Notas LIKE '*" & EscapeSQL(p_Google) & "*')"
    End If
    ...
    Do While Not rs.EOF
        ...
        vm.IDExpediente = Nz(rs!IDExpediente, "")
    ...
    Set GetListadoFiltradoSQL = colVM
    Exit Function
    
errores:
    If Err.Number <> 1000 Then
        p_Error = "Error en GetListadoFiltradoSQL: " & Err.Description
    End If
    Set GetListadoFiltradoSQL = Nothing
End Function
...
Public Function SincronizarCache(Optional ByRef p_Error As String) As Boolean
    ...
    Dim colFaltantes As Scripting.Dictionary
    Dim colSobrantes As Scripting.Dictionary
    Dim ID As Long
    ...
    Set rsFuente = db.OpenRecordset("SELECT IDNoConformidad FROM TbNoConformidades", dbOpenSnapshot)
    Set rsCache = db.OpenRecordset("SELECT IDNoConformidad FROM TbCacheListadoNC", dbOpenSnapshot)
    
    If Not (rsFuente.BOF And rsFuente.EOF) Then
        Do While Not rsFuente.EOF
            If Not ExisteEnRecordset(rsCache, rsFuente!IDNoConformidad) Then
                If colFaltantes Is Nothing Then
                    Set colFaltantes = New Scripting.Dictionary
                    colFaltantes.CompareMode = TextCompare
                End If
                colFaltantes.Add CStr(rsFuente!IDNoConformidad), rsFuente!IDNoConformidad
            End If
            rsFuente.MoveNext
        Loop
    End If
    
    If Not (rsCache.BOF And rsCache.EOF) Then
        Do While Not rsCache.EOF
            If Not ExisteEnRecordset(rsFuente, rsCache!IDNoConformidad) Then
                If colSobrantes Is Nothing Then
                    Set colSobrantes = New Scripting.Dictionary
                    colSobrantes.CompareMode = TextCompare
                End If
                colSobrantes.Add CStr(rsCache!IDNoConformidad), rsCache!IDNoConformidad
            End If
            rsCache.MoveNext
        Loop
    End If
    ...
    If Not colSobrantes Is Nothing Then
        For Each VarItem In colSobrantes
            db.Execute "DELETE FROM TbCacheListadoNC WHERE IDNoConformidad = " & VarItem
        Next
    End If
    
    If Not colFaltantes Is Nothing Then
        For Each VarItem In colFaltantes
            If Not RegenerarRegistro(CLng(VarItem)) Then
                Exit Function
            End If
        Next
    End If
...
errores:
    p_Error = "Error en SincronizarCache: " & Err.Description
    SincronizarCache = False
End Function

Private Function ExisteEnRecordset(rs As Dao.Recordset, ID As Long) As Boolean
    
    If rs.BOF And rs.EOF Then
        ExisteEnRecordset = False
        Exit Function
    End If
    
    rs.MoveFirst
    Do While Not rs.EOF
        If rs!IDNoConformidad = ID Then
            ExisteEnRecordset = True
            Exit Function
        End If
        rs.MoveNext
    Loop
...
Public Function RegenerarRegistro(ByVal p_IDNoConformidad As Long, Optional ByRef p_Error As String) As Boolean
    Dim db As Dao.Database
    ...
    db.Execute "DELETE FROM TbCacheListadoNC WHERE IDNoConformidad = " & p_IDNoConformidad
    
    SQL = "SELECT n.IDNoConformidad, n.CodigoNoConformidad, n.IDExpediente, n.Descripcion, n.Notas, n.Estado, " & _
          "n.FechaApertura, n.FECHACIERRE, n.IDTipo, n.RequiereControlEficacia, n.ControlEficacia, " & _
          "n.ResponsableTelefonica, n.RESPONSABLECALIDAD, n.ACR, " & _
          "e.Nemotecnico, e.CodExp " & _
          "FROM TbNoConformidades n " & _
          "LEFT JOIN TbExpedientes e ON n.IDExpediente = e.IDExpediente " & _
          "WHERE n.IDNoConformidad = " & p_IDNoConformidad
    Set rsNC = db.OpenRecordset(SQL, dbOpenSnapshot)
    
    If rsNC.EOF Then
        p_Error = "No se encontró la NC con ID " & p_IDNoConformidad
        rsNC.Close
        Exit Function
    End If
    
    Set rsCache = db.OpenRecordset("TbCacheListadoNC", dbOpenDynaset)
    rsCache.AddNew
    rsCache!IDNoConformidad = rsNC!IDNoConformidad
    rsCache!CodigoNoConformidad = Nz(rsNC!CodigoNoConformidad, "")
    rsCache!IDExpediente = Nz(rsNC!IDExpediente, 0)
    rsCache!Descripcion = Nz(rsNC!Descripcion, "")
    rsCache!Notas = Nz(rsNC!Notas, "")
    rsCache!Estado = Nz(rsNC!Estado, "")
    rsCache!FechaApertura = rsNC!FechaApertura
    rsCache!FECHACIERRE = rsNC!FECHACIERRE
    rsCache!IDTipo = Nz(rsNC!IDTipo, 0)
    rsCache!RequiereControlEficacia = Nz(rsNC!RequiereControlEficacia, "")
    rsCache!ControlEficacia = Nz(rsNC!ControlEficacia, "")
    rsCache!ResponsableTelefonica = Nz(rsNC!ResponsableTelefonica, "")
    rsCache!RESPONSABLECALIDAD = Nz(rsNC!RESPONSABLECALIDAD, "")
    rsCache!ACR = Nz(rsNC!ACR, "")
    If Not IsNull(rsNC!Nemotecnico) Then rsCache!Nemotecnico = rsNC!Nemotecnico
    If Not IsNull(rsNC!CodExp) Then rsCache!CodExp = rsNC!CodExp
    rsCache!Cerrada = IIf(IsDate(rsNC!FECHACIERRE), True, False)
    rsCache.Update
...
errores:
    p_Error = "Error en RegenerarRegistro: " & Err.Description
