...
Public Function Test_E2E_EnvConfig_ResuelveSandboxSeguro_Atomic() As String
    ...
    Set logs = TestHelper.NewLogs
    Set db = CurrentDb
    Set rs = db.OpenRecordset("SELECT * FROM TbConfiguracionBackends WHERE ID = 1", dbOpenSnapshot)
    ...
    TestHelper.AddLog logs, "EnPruebas/EnDesarrollo=" & enPruebas
    ...
    Call TestHelper.AssertTrue(enPruebas = "Sí" Or enPruebas = "No", "EnPruebas/EnDesarrollo debe ser 'Sí' o 'No'", logs, assertError)
    If assertError <> "" Then GoTo Fail
