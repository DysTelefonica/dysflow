...
export const TOOL_DESCRIPTIONS: Record<DysflowMcpToolName, string> = {
  ...
  export_all:
    "Mirror ALL VBA modules from the binary to the source tree. diff:true reports per-file drift without writing. prune:true DELETES orphaned managed source files (.bas/.cls/.form.txt/.report.txt) absent from the binary; prune is rejected together with filter. Read-only on the binary. Set verbose:true to detect silent truncation/encoding-loss — the response includes per-module {source, destination, line-count + sha256} and a truncated flag.",
  import_modules:
  ...
  import_all:
    "Import the entire source tree into the Access binary (mutates the binary; write-gated). RESERVED for whole-project resync (initial setup, disaster recovery, post-fork reconciliation). NOT a fallback for import_modules — if you have an explicit list of modules, use import_modules with moduleNames; if you pass moduleNames: [] to import_modules, it is a no-op plan, it does NOT expand to this tool. Supports compile:true and dryRun:true (plan mode). Set verbose:true to detect silent truncation/encoding-loss (issue #752) — same per-module verbose field as import_modules.",
  list_objects:
