...
export const TOOL_DESCRIPTIONS: Record<DysflowMcpToolName, string> = {
  ...
  export_modules:
    "Export specific VBA modules from the Access binary to the on-disk source tree. Scope with moduleNames (exact) or filter (substring). Read-only on the binary. destinationRoot is honored only when accessPath is also passed; otherwise the project source root is used. For a full mirror use export_all. With verbose:true, the response adds per-module binary-before/file-after snapshots and the canonical VBA semantic classification, actionability, recommendation, reason, and classifier-rules version; verbose is absent otherwise.",
  export_all:
    "Mirrors all VBA modules from the binary to the source tree when apply:true is explicit; omission or apply:false returns a non-writing plan. prune:true deletes orphaned managed source files (.bas/.cls/.form.txt/.report.txt) only after a clean export and is rejected together with filter. Read-only on the binary. With verbose:true, the response adds the same per-module binary-before/file-after snapshots and canonical semantic verdict as export_modules; verbose is absent otherwise.",
  import_modules:
    "Import specific source modules INTO the Access binary (mutates the binary; write-gated). USE THIS whenever you know the exact list of modules you want to import — accepts long lists (20-30+ modules, no hard cap) and emits a STRUCTURED PER-MODULE REPORT so callers can pinpoint which entry failed and why. Each module entry carries {module, status, phase (locate-source|remove-existing|import), error:{code, message, machine, user}, durationMs}. importMode controls merge vs replace. Mutations persist via save-only (acCmdSaveAllModules = RunCommand 280); the runtime does NOT compile — the human compiles in Access (Debug > Compile) before re-running tests. dryRun:true returns a plan without writing. With verbose:true, successful entries add source/destination snapshots plus the canonical VBA semantic classification, actionability, recommendation, reason, and classifier-rules version. IMPORT_TRUNCATED stays fatal and rolls back. An empty moduleNames array is treated as an explicit no-op plan (NOT a silent fallback to import-all). When Access holds an exclusive lock on the .accdb, the per-module error.code surfaces ACCESS_DATABASE_LOCKED with machine/user when parseable. Defensive validations surface as typed error.code values: VB_NAME_MISMATCH, DUPLICATE_OPTION_DIRECTIVE, IMPORT_TRUNCATED.",
  import_all:
  ...
  test_vba:
    "Run VBA test procedures and report pass/fail per atom — the project's green gate. Select tests with proceduresJson (explicit list), filter (name substring) or testsPath. Requires a compiled project: import first, then the human compiles in Access (Debug > Compile), then run test_vba. The dysflow runtime does NOT compile; consumers compile in Access. Missing or empty allowedProcedures imposes no restriction; a non-empty list is an opt-in whitelist.",
  ...
  verify_code:
    "Compare the on-disk source against the VBA source exported live from the binary. Read-only and dry-run. moduleNames is a true focused export request (Access exports only requested modules, then the compare is filtered to the same modules). The default response is compact: read recommendedAction, actionableOk, summaryStructured.actionableTotal/nonActionableTotal, and bulkImportable/bulkExportable. summaryByCategory and nonActionableByCategory are aggregate counts only; never infer module membership from them. Raw different[], diffs[], matched/missing arrays, actionableDifferent[], nonActionableDifferent[], moduleCounts, and summaryUnits require diagnostic:true; use those classified arrays when membership matters, and note that one logical Access form/report can emit independently classified .cls and .form.txt entries. The opt-in also enables per-module snippets. USE BEFORE AND AFTER import_modules / import_all to confirm Unicode characters round-trip cleanly (no mojibake drift) and that the per-module result matches the binary. Timeout failures are phase-aware: export stalls return VBA_MANAGER_TIMEOUT; preflight and compare stalls return VERIFY_CODE_PHASE_TIMEOUT with details.phase, details.moduleName/details.moduleNames, details.operationTimeoutMs and details.phaseTimeoutMs. Export-phase errors also carry details.durationMs; if post-timeout Access orphan cleanup exceeds its own bound, details.cleanupTimedOut:true and details.cleanupTimeoutMs are set on the parent error. Folding is string-aware. strict:true does a byte-exact compare; diagnostic:true respects strict mode while exposing its byte-exact evidence.",
  delete_module:
