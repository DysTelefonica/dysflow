...
const NO_INPUT_SCHEMA: JsonObjectSchema = {
	type: "object",
...
const CONTEXT_PROPERTIES: Record<string, JsonSchemaProperty> = {
	contextId: {
		type: "string",
		description:
			"Optional project/context id for traceability. Paths and roots still come from .dysflow/project.json unless explicitly overridden by a tool that supports overrides.",
	},
	projectId: {
		type: "string",
		description:
			"Optional project id alias for traceability. Paths and roots still come from .dysflow/project.json unless explicitly overridden by a tool that supports overrides.",
	},
};
...
export type DysflowMcpServices = {
	...
	legacyToolService?: {
		execute(
			toolName: LegacyDysflowMcpToolName,
			input: unknown,
	...
	if (property.type === "object" && isRecord(value)) {
		if (property.additionalProperties === false) {
			for (const key of Object.keys(value)) {
				if (property.properties?.[key] === undefined)
					return `${path}.${key} is not allowed.`;
			}
		}
		for (const [key, childProperty] of Object.entries(
			property.properties ?? {},
		)) {
...
function legacySchemaForTool(
	name:
		| LegacyDysflowMcpToolName
		| "run_vba"
		| "query_sql"
	...
	const properties: Record<string, JsonSchemaProperty> = {
		...
		accessPath: {
			type: "string",
			description:
				"Optional override for Access frontend database path. Normal calls use .dysflow/project.json.",
		},
		...
		backendPath: {
			type: "string",
			description:
				"Optional override for Access backend database path. Normal calls use .dysflow/project.json.",
		},
		...
		destinationRoot: {
			type: "string",
			description:
				"Optional override for source/export root. Normal calls use .dysflow/project.json destinationRoot, typically src.",
		},
		...
		projectRoot: {
			type: "string",
			description:
				"Optional override for project root. Normal calls use the active repo/worktree and .dysflow/project.json.",
		},
		procedureName: {
			type: "string",
		...
		rootPath: {
			type: "string",
			description:
				"Optional override for root directory. Normal calls use .dysflow/project.json destinationRoot.",
		},
		strict: {
			type: "boolean",
	...
	if (name === "run_vba") {
		return {
			...
			additionalProperties: false,
			properties: {
				procedureName: properties.procedureName,
				argsJson: {
					type: "string",
					description: "JSON encoded argument array.",
				},
			},
		};
	}
...
async function handleValidatedLegacyQuery<TData>(
	input: unknown,
	schema: JsonObjectSchema,
	...
	const currentTools: DysflowMcpTool[] = [
		{
			...
			handler: async (input) => {
				...
				return translateCoreResultToMcpContent(
					await services.vbaService.execute(input as AccessVbaRequest),
				);
			},
		},
		{
			...
			handler: async (input) => {
				...
				if (validation !== undefined) return invalidInput(validation);
				return translateCoreResultToMcpContent(
					await services.queryService.execute(input as AccessQueryRequest),
				);
			},
		},
		{
			...
			handler: async (input) => {
				...
				if (validation !== undefined) return invalidInput(validation);
				return translateCoreResultToMcpContent(
					await services.diagnosticsService.run(
						input as AccessDiagnosticsRequest,
					),
				);
			},
		},
		{
			...
			handler: async () => {
				const registry =
					services.operationRegistry ?? getDefaultAccessOperationRegistry();
				return translateCoreResultToMcpContent(
					successResult<readonly AccessOperationRecord[]>(
						await registry.listRecent({ limit: 50 }),
					),
				);
			},
		},
		{
			...
			description:
				"Safely cleanup a registered Access operation by operationId and accessPath.",
...
function appendLegacyCompatibilityTools(
	...
	add({
		...
		description:
			"Legacy-compatible alias for listing Dysflow Access operations.",
		inputSchema: NO_INPUT_SCHEMA,
		handler: async () => {
			const registry =
				services.operationRegistry ?? getDefaultAccessOperationRegistry();
	...
	add({
		...
		handler: async (input) => {
			const validation = validateInput(
				input,
				legacySchemaForTool("cleanup_access_operation"),
			);
			if (validation !== undefined) return invalidInput(validation);
			if (services.cleanupService === undefined) {
				return {
	...
	add({
		name: "run_vba",
		description:
			"Legacy-compatible alias for executing a public VBA procedure.",
	...
	add({
		name: "exec_sql",
		description:
			"Legacy-compatible alias for executing guarded Access SQL writes.",
		inputSchema: legacySchemaForTool("exec_sql"),
		handler: async (input) =>
			handleValidatedLegacyQuery(input, legacySchemaForTool("exec_sql"), () =>
				services.queryService.execute(
					toLegacyWriteFixtureRequest("exec_sql", input),
				),
			),
	});
	add({
		name: "run_script",
		description:
			"Legacy-compatible alias for executing a guarded Access script.",
		inputSchema: legacySchemaForTool("run_script"),
		handler: async (input) =>
			handleValidatedLegacyQuery(input, legacySchemaForTool("run_script"), () =>
				services.queryService.execute(
					toLegacyWriteFixtureRequest("run_script", input),
				),
			),
	});
	add({
		name: "create_table",
		description:
			"Legacy-compatible alias for creating a table through guarded Access writes.",
		inputSchema: legacySchemaForTool("create_table"),
		handler: async (input) =>
			handleValidatedLegacyQuery(
				input,
				legacySchemaForTool("create_table"),
				() =>
					services.queryService.execute(
						toLegacyWriteFixtureRequest("create_table", input),
					),
			),
	});
	add({
		name: "drop_table",
		description:
			"Legacy-compatible alias for dropping a table through guarded Access writes.",
		inputSchema: legacySchemaForTool("drop_table"),
		handler: async (input) =>
			handleValidatedLegacyQuery(input, legacySchemaForTool("drop_table"), () =>
				services.queryService.execute(
					toLegacyWriteFixtureRequest("drop_table", input),
				),
			),
	});
	add({
		name: "seed_fixture",
		description:
			"Legacy-compatible alias for seeding fixtures through guarded Access writes.",
		inputSchema: legacySchemaForTool("seed_fixture"),
		handler: async (input) =>
			handleValidatedLegacyQuery(
				input,
				legacySchemaForTool("seed_fixture"),
				() =>
					services.queryService.execute(
						toLegacyWriteFixtureRequest("seed_fixture", input),
					),
			),
	});
	add({
		...
		description:
			"Legacy-compatible alias for tearing down fixtures through guarded Access writes.",
		inputSchema: legacySchemaForTool("teardown_fixture"),
		handler: async (input) =>
			handleValidatedLegacyQuery(
				input,
				legacySchemaForTool("teardown_fixture"),
				() =>
					services.queryService.execute(
						toLegacyWriteFixtureRequest("teardown_fixture", input),
					),
			),
	});
...
function createLegacyDispatchTool(
	...
	return {
		...
		handler: async (input) => {
			...
			if (validation !== undefined) return invalidInput(validation);
			if (
				isVbaSyncSliceTool(name) &&
				services.legacyToolService !== undefined
			) {
				return translateCoreResultToMcpContent(
			...
			if (isQueryMaintenanceSliceTool(name)) {
				return translateCoreResultToMcpContent(
					await services.queryService.execute(
						toLegacyMaintenanceRequest(name, input),
					),
				);
			}
			if (isQuerySliceTool(name)) {
				return translateCoreResultToMcpContent(
					await services.queryService.execute(
						toLegacyQueryRequest(name, input),
					),
				);
			}
			if (isWriteFixtureSliceTool(name)) {
				return translateCoreResultToMcpContent(
					await services.queryService.execute(
						toLegacyWriteFixtureRequest(name, input),
					),
				);
			}
			return {
...
function isWriteFixtureSliceTool(name: LegacyDysflowMcpToolName): boolean {
	return (LEGACY_WRITE_FIXTURE_SLICE_TOOL_NAMES as readonly string[]).includes(
		name,
	);
}
...
function parseLegacyArgsJson(
	argsJson: string | undefined,
): LegacyArgsJsonParseResult {
...
function toLegacyQueryRequest(
	...
	const columnName =
		stringValue(params.columnName) ?? stringValue(params.column);
	return {
		...
		columnName,
		backendPath:
			stringValue(params.backendPath) ?? stringValue(params.comparePath),
		rootPath: stringValue(params.rootPath) ?? stringValue(params.directory),
		databasePath:
			stringValue(params.databasePath) ?? stringValue(params.sourcePath),
		...
		queryDefinitions:
			queryDefinitionsValue(params.queryDefinitions) ??
			queryDefinitionsValue(params.queries),
	};
}
...
function toLegacyWriteFixtureRequest(
	...
	return {
		...
		allowTables:
			stringArrayValue(params.allowTables) ??
			singleStringArrayValue(params.allowTable),
		denyTables:
			stringArrayValue(params.denyTables) ??
			singleStringArrayValue(params.denyTable),
	};
}
