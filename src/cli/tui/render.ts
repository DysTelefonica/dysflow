...
export function renderDashboard(options: DashboardRenderOptions): string {
  const status = buildVersionStatus(options.localVersion, options.latestVersion);
  const lines = [renderLogo(), `local: ${status.localVersion}   latest: ${status.latestVersion}`];
  ...
  lines.push(
    "",
    ...MENU_OPTIONS.map((option, index) => renderOption(option, index === options.cursor)),
  );
  lines.push("", "↑/↓: move • enter: select • q: exit");
  return lines.join("\n");
}

export function renderIntegrationSelection(options: IntegrationSelectionRenderOptions): string {
...
function renderCheckbox(label: string, checked: boolean, focused: boolean): string {
