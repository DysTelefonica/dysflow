...
| `MCP_INPUT_INVALID` | The request does not match the tool schema. Structured envelopes may identify one omitted required field as `error.missingParam` and rejected flags as `error.rejectedFlag` / `error.rejectedFlags`; telemetry aggregates those names only, never argument values. | Read the live tool schema and replace unsupported or missing fields. For `form_set_property`, use `property`, not `propertyName`. |
