...
| `SANDBOX_ONLY` | `run_script` received an explicit Access target outside the active worktree resolved from the physical project configuration. | Copy the database into the current worktree sandbox or select the correct project before retrying. |
| `CONFIRMATION_REQUIRED` | A process-control operation needs explicit human confirmation before dispatch, including selecting a specific orphan PID for forced cleanup. | Ask the human to confirm the exact operation, then retry with the tool's confirmation fields. |
